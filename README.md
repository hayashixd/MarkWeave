...
### 5. 書いた後、AIなしで文章を磨ける（Prose Lint）

冗長な表現・文体の混在・長すぎる文を自動検出。修正候補を提示するので、推敲の手間を減らせる。
Claude API キー不要。すべてローカルで動く。

![Prose Lint demo](doc-public/demo-gifs/prose-lint.gif)
...
| 文章スタイル Lint（Prose Lint） | 文の長さ・文体混在・冗長表現・助詞の重複を AI 不要でローカルチェック。修正案付き（`Ctrl+Shift+8`） |
...
> **30日間無料試用**: インストール後30日間はライセンスなしで全機能を試用できます。試用期間終了後はライセンスキーの入力が必要です。

---

### Windows インストール時の SmartScreen 警告について

`.msi` を実行すると「**Windows によって PC が保護されました**」という警告が表示されることがあります。

**これは MarkWeave が危険なソフトウェアだからではありません。**
個人開発の新しいアプリのため、Windows 側の実績データがまだ蓄積されていない段階で表示される警告です。

**インストールを続ける手順:**

1. 警告画面に表示されている「**詳細情報**」をクリック
2. 画面下部に「**実行**」ボタンが現れるのでクリック
3. 通常のインストーラーが起動します

**ファイルの整合性を確認したい場合（任意）:**

各リリースの SHA256 チェックサムを [Releases ページ](../../releases) に掲載しています。
ダウンロード後に以下のコマンドで照合できます。
