...
**更新ポリシーの全体像**（Phase ごとの対応範囲）:

| 対象 | Phase 1〜2 | Phase 3 | Phase 7 |
|-----|-----------|---------|---------|
| 移動ファイル **自身の** 画像パス (`![](./assets/img.png)`) | ❌ 未対応 | **✅ 自動更新**（`image-design.md §3`） | — |
| **他のファイル** から移動ファイルへの `.md` リンク | ❌ 未対応 | ❌ 未対応 | **✅ 確認ダイアログ付きで更新** |
| **他のファイル** から移動ファイルへの画像リンク | ❌ 未対応 | ❌ 未対応 | **✅ 確認ダイアログ付きで更新** |

> **MVP（Phase 1〜2）のポリシー**: ファイルをリネーム・移動しても、**他のファイル内の相対リンクは自動更新しない**。
> Phase 3 以降は **移動ファイル自身の画像パス** のみ自動更新される（画像 D&D 機能と同時実装）。

**Phase 3 での移動時の UX**:
- ファイル移動時に「他のファイルのリンクは更新されません（Phase 7 以降で対応予定）」を通知バナーで案内する
- 移動後、当該ファイルを開いたときに画像がリンク切れしていないことを確認できる
