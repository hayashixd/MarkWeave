...
| タブで開いているファイルが変更、かつ編集中（未保存あり）| 競合ダイアログを表示（§4.2.1 参照）|

#### 4.2.1 競合解決ダイアログ（未保存変更がある場合）

外部でファイルが変更され、かつエディタに未保存の変更がある場合は以下のダイアログを表示する。
...
| 操作 | 挙動 |
|------|------|
| ファイルを再読込 | ディスクの内容でエディタを上書き。未保存変更は破棄（確認なし）|
| ローカル変更を保持 | ディスクの変更を無視。エディタの内容をそのまま継続編集 |
| 差分を表示 | スプリットビューでディスク版（左）とエディタ版（右）を表示。Phase 4 以降の機能 |

**カーソル・スクロール位置の扱い**:
- 「ファイルを再読込」後は、変更前のカーソル位置（行番号ベース）に復元を試みる。
  行数が変わっていた場合は末尾に移動する。
- 「ローカル変更を保持」後はカーソル・スクロール位置を変更しない。

**自動保存中の競合（書き込み競合）**:
- 自動保存のトリガー時にファイルのタイムスタンプ（または SHA-256 ハッシュ）を検証し、
  外部変更を検出した場合は自動保存を中断して上記ダイアログを表示する。
- ユーザーが「ローカル変更を保持」を選択した場合は、改めて手動保存（Ctrl+S）を促す通知を表示する。
