...
**bold _bold-italic_ bold**
```
（bold がより広いスパンを持つため、bold が外側 → `**..._..._...**` の形式で出力）

#### `***text***`（完全一致スパン）の具体的な判定テスト

TipTap→mdast 変換で `***` を正しく出力するかを検証するテスト群。
詳細アルゴリズムは `markdown-tiptap-conversion.md` セクション2.3を参照。

```typescript
describe('bold+italic 複合マークの出力形式', () => {
  // ケース1: 完全一致スパン → ***
  test('bold と italic が同一スパン → ***text***', () => {
    const md = '***bold and italic***\n';
    expect(roundTrip(md)).toBe(md);
  });

  // ケース2: bold が外側（より広いスパン）→ **..._..._...**
  test('bold が外側、italic が部分的 → **outer _inner_ outer**', () => {
    const md = '**bold _bold-italic_ bold**\n';
    expect(roundTrip(md)).toBe(md);
  });

  // ケース3: italic が外側（より広いスパン）→ *...**...**...*
  test('italic が外側、bold が部分的 → *outer **inner** outer*', () => {
    const md = '*italic **italic-bold** italic*\n';
    expect(roundTrip(md)).toBe(md);
  });

  // ケース4: `__bold__` の正規化 → `**bold**` に変換してから処理
  test('__bold__ と *italic* の組み合わせは *** に正規化', () => {
    // 入力が `*__text__*` の場合、`__` → `**` 正規化後に *** になる
    const input  = '*__bold and italic__*\n';
    const output = '***bold and italic***\n'; // ← 正規化後
    expect(roundTrip(input)).toBe(output);
  });

  // ケース5: スパンが隣接しているが一致しない
  test('bold+italic の後に italic のみ → 別々に出力', () => {
    const md = '***bi*** *i*\n';
    expect(roundTrip(md)).toBe(md);
  });
});
...
**TipTap JSON での検証**（スナップショットテスト）:

```typescript
describe('bold+italic TipTap JSON 構造', () => {
  test('*** から変換した TipTap JSON は bold+italic マーク両持ち', () => {
    const json = markdownToTipTap('***text***\n');
    const textNode = json.content[0].content[0]; // paragraph > text
    expect(textNode.marks?.map((m: { type: string }) => m.type).sort())
      .toEqual(['bold', 'italic']);
  });

  test('**..._..._...** では3種類のテキストノードが生成される', () => {
    const json = markdownToTipTap('**bold _bold-italic_ bold**\n');
    const paragraph = json.content[0];
    // "bold " → [bold], "bold-italic" → [bold, italic], " bold" → [bold]
    expect(paragraph.content).toHaveLength(3);
    expect(paragraph.content[0].marks?.map((m: { type: string }) => m.type)).toEqual(['bold']);
    expect(paragraph.content[1].marks?.map((m: { type: string }) => m.type).sort())
      .toEqual(['bold', 'italic']);
    expect(paragraph.content[2].marks?.map((m: { type: string }) => m.type)).toEqual(['bold']);
  });
});
```
...
### 4.8 Link 拡張の `target` / `rel` 属性によるラウンドトリップ破壊

TipTap `@tiptap/extension-link` がデフォルトで付与する `target="_blank"` と `rel="noopener noreferrer nofollow"` が、Markdown 出力に漏れるケース。
...
<!-- ❌ 誤った出力（target が漏れた場合） -->
[example](https://example.com){target="_blank" rel="noopener noreferrer nofollow"}
```

**テストケース**

```typescript
describe('Link target 属性のラウンドトリップ', () => {
  test('target/rel 属性を持たないシンプルリンク', () => {
    const md = '[example](https://example.com)\n';
    expect(roundTrip(md)).toBe(md);
  });

  test('title 付きリンク', () => {
    const md = '[example](https://example.com "説明")\n';
    expect(roundTrip(md)).toBe(md);
  });

  test('リンクに target/rel が混入しない（TipTap JSON 検証）', () => {
    const json = markdownToTipTap('[example](https://example.com)\n');
    const linkMark = json.content[0].content[0].marks
      ?.find((m: { type: string }) => m.type === 'link');
    expect(linkMark?.attrs?.target).toBeNull();
    expect(linkMark?.attrs?.rel).toBeNull();
  });

  test('mdast → TipTap → mdast でリンク属性が欠損しない', () => {
    const md = '[リンクテキスト](https://example.com/path?q=1#anchor)\n';
    expect(roundTrip(md)).toBe(md);
  });

  test('空の title は出力しない', () => {
    // title が null または "" のとき、Markdown に "" を追加しない
    const md = '[example](https://example.com)\n';
    expect(roundTrip(md)).toBe(md);
  });
});
...
- [ ] `Link.configure({ HTMLAttributes: { target: null, rel: null } })` が設定されている
- [ ] シリアライザが `mark.attrs.href` と `mark.attrs.title` のみを使用している
- [ ] 全 Link フィクスチャのラウンドトリップが通過する

---

### 4.9 IME Composition 中のシリアライズ制御（テスト観点）

IME 入力中に `tiptapToMarkdown()` が呼ばれると未確定テキストが混入する。
詳細仕様・実装コードは `markdown-tiptap-conversion.md` セクション9 を参照。

**ユニットテストで確認すべき観点**

```typescript
describe('isComposing ガード', () => {
  test('isComposing = true のとき serializeToMarkdown が null を返す', () => {
    const editor = new MarkdownEditor(/* mock */);
    // 内部フラグを強制セット（テスト用の setComposingForTest メソッドを用意する）
    editor.setComposingForTest(true);
    expect(editor.serializeToMarkdown()).toBeNull();
  });

  test('isComposing = false のとき正常に文字列を返す', () => {
    const editor = createEditorWithContent('テスト');
    editor.setComposingForTest(false);
    expect(editor.serializeToMarkdown()).toBe('テスト\n');
  });
});

describe('自動保存 scheduleSave のガード', () => {
  test('isComposing = true のとき saveTimer が起動しない', () => {
    const mockEditor = { isComposing: true, serializeToMarkdown: () => null };
    const writeSpy = jest.fn();

    scheduleSave(mockEditor as any, '/tmp/test.md', writeSpy);
    jest.runAllTimers();

    expect(writeSpy).not.toHaveBeenCalled();
  });

  test('compositionend 後に flushPendingOperations が保存を実行する', () => {
    const mockEditor = { isComposing: true, serializeToMarkdown: () => null };
    const writeSpy = jest.fn();

    // composition 中に scheduleSave を呼ぶ（pendingSave フラグが立つ）
    scheduleSave(mockEditor as any, '/tmp/test.md', writeSpy);

    // composition 終了
    (mockEditor as any).isComposing = false;
    (mockEditor as any).serializeToMarkdown = () => '確定テキスト\n';
    flushPendingOperations(mockEditor as any, '/tmp/test.md', writeSpy);

    jest.runAllTimers();
    expect(writeSpy).toHaveBeenCalledWith('/tmp/test.md', '確定テキスト\n');
  });
});
```

**E2E テスト（Playwright）で確認すべき観点**

| テスト | 確認事項 |
|---|---|
| 日本語入力 → ファイル保存 | 確定後のテキストのみが保存されること |
| composition 中の Ctrl+Z | undoが composition 全体を1操作として扱うこと |
| composition 中の保存ボタン | 保存がスキップされ、確定後に再試行されること |
| 長文日本語入力のラウンドトリップ | 変換前後で日本語テキストが一致すること |
