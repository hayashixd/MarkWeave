...
脚注の逆変換は以下の2ステップで行う：
1. `footnoteRef` インラインノードを mdast `footnoteReference` に変換する
2. doc attrs の `footnotes` マップから mdast `footnoteDefinition` ノードを生成し、本文ノードの後に追加する

```typescript
// src/core/converter/tiptap-to-mdast.ts

/**
 * TipTap doc → mdast への脚注変換。
 * doc.attrs.footnotes からすべての脚注定義を末尾ブロックとして生成する。
 */
function convertDocToMdast(doc: TipTapDoc): MdastRoot {
  // Step 1: 本文ノードを通常変換
  const bodyNodes: MdastNode[] = doc.content?.map(convertBlock) ?? [];

  // Step 2: doc attrs.footnotes から footnoteDefinition を末尾に追加
  const footnotes: Record<string, string> = doc.attrs?.footnotes ?? {};
  const footnoteDefinitions: MdastNode[] = Object.entries(footnotes).map(
    ([identifier, content]) => ({
      type: 'footnoteDefinition',
      identifier,
      label: identifier,
      children: parseMarkdownToMdastInline(content), // 脚注本文を再パース
    })
  );

  return {
    type: 'root',
    children: [...bodyNodes, ...footnoteDefinitions],
  };
}
...
case 'footnoteRef':
  return {
    ...
    label: node.attrs.identifier,
  };
```

**注意**: 脚注番号の順序は `identifier` のキー順ではなく、
**本文中に `footnoteRef` が登場する順序** で出力する（remark-stringify のデフォルト動作と一致）。
`convertDocToMdast` で `bodyNodes` を走査して登場順に `identifier` を収集し、
その順序で `footnoteDefinitions` を並べること。
...
```typescript
// TipTap 組み込みの ListItem を extend() で拡張して spread attr を追加する。
// Node.create({ name: 'listItem' }) で新規作成すると組み込みの ListItem と競合するため、
// 必ず extend() を使うこと。
...
const ListItem = TipTapListItem.extend({
  addAttributes() {
    return {
      ...this.parent?.(), // 既存の attrs を継承
      spread: {
        default: false,
        parseHTML: (el) => el.getAttribute('data-spread') === 'true',
        renderHTML: (attrs) =>
          attrs.spread ? { 'data-spread': 'true' } : {},
      },
    };
  },
});
...
| レベル | 名称 | 意味 |
|--------|------|------|
| **A** | 完全保持（Lossless） | 変換前後で Markdown ソースが一致する（正規化ルールを除く） |
| **B** | 正規化（Normalized） | 意味・内容は完全に保持されるが、表記形式が正規化される（例: `_italic_` → `*italic*`） |
| **C** | 近似変換（Approximated） | 意味的に等価だが一部の情報が失われる可能性がある |
| **D** | 不透明保持（Opaque） | TipTap では解釈せず生文字列として保持。編集不可だがラウンドトリップは保証 |
| **X** | 非対応（Unsupported） | 変換時に失われる。ユーザーに警告を表示する |

### 8.2 ブロック要素の変換サポートマトリクス

| Markdown 要素 | TipTap ノード | レベル | 正規化ルール / 注意点 |
|---|---|---|---|
| ATX 見出し（`# H1`〜`###### H6`） | `heading` | **A** | — |
| Setext 見出し（`===`, `---`） | `heading` | **B** | ATX 形式に正規化 |
| 段落 | `paragraph` | **A** | — |
| 引用ブロック（`>`） | `blockquote` | **A** | 入れ子も再帰的に保持 |
| フェンスコードブロック（` ``` `） | `codeBlock` | **A** | 言語タグ保持 |
| インデントコードブロック（4スペース） | `codeBlock` | **B** | フェンス形式に正規化 |
| チルダフェンス（`~~~`） | `codeBlock` | **B** | バックティック形式に正規化 |
| 順序なしリスト（`-`/`*`/`+`） | `bulletList` | **B** | マーカーを `-` に統一 |
| 順序付きリスト（`1.`〜） | `orderedList` | **A** | `start` 属性保持 |
| タスクリスト（`- [ ]`/`- [x]`） | `taskItem` | **A** | null/false/true 三値を保持 |
| タイトリスト（項目間に空行なし） | `listItem` | **A** | `spread: false` を attrs で保持 |
| ルーズリスト（項目間に空行あり） | `listItem` | **A** | `spread: true` を attrs で保持 |
| GFM テーブル | `table` | **A** | align を各セル attrs に分配して保持 |
| 水平線（`***`/`---`/`___`） | `horizontalRule` | **B** | `---` に正規化 |
| YAML フロントマター | `frontmatter`（カスタム） | **A** | 先頭ノードとして保持 |
| 数式ブロック（`$$...$$`） | `mathBlock`（カスタム） | **A** | remark-math 使用 |
| ブロック生HTML（`<div>` 等） | `rawHtmlBlock`（カスタム） | **D** | 生文字列として不透明保持 |
| HTMLコメント（`<!-- -->`） | `rawHtmlBlock`（カスタム） | **D** | 生文字列として不透明保持 |

### 8.3 インライン要素の変換サポートマトリクス

| Markdown 要素 | TipTap マーク/ノード | レベル | 正規化ルール / 注意点 |
|---|---|---|---|
| 太字（`**bold**`） | `bold` mark | **A** | — |
| 太字アンダーバー（`__bold__`） | `bold` mark | **B** | `**bold**` に正規化 |
| 斜体（`*italic*`） | `italic` mark | **A** | — |
| 斜体アンダーバー（`_italic_`） | `italic` mark | **B** | `*italic*` に正規化 |
| 太字+斜体（`***text***`） | `bold` + `italic` marks | **A** | — |
| 取り消し線（`~~text~~`） | `strike` mark | **A** | remark-gfm 使用 |
| インラインコード（`` `code` ``） | `code` mark | **A** | — |
| インラインリンク（`[text](url)`） | `link` mark | **A** | `href`, `title` 保持 |
| 参照リンク（`[text][ref]`） | `link` mark | **C** | インラインリンクに展開（参照定義は消える） |
| オートリンク（`<https://...>`） | `link` mark | **B** | インラインリンクに正規化 |
| 画像（`![alt](src)`） | `image` ノード | **A** | `src`, `alt`, `title` 保持 |
| ハードブレーク（行末スペース2つ） | `hardBreak` | **B** | バックスラッシュ形式に正規化 |
| ハードブレーク（`\` 末尾） | `hardBreak` | **A** | — |
| 脚注参照（`[^1]`） | `footnoteRef`（カスタム） | **A** | doc attrs に定義を集約 |
| インライン数式（`$...$`） | `mathInline`（カスタム） | **A** | remark-math 使用 |
| インライン生HTML（`<span>` 等） | `rawHtmlInline`（カスタム） | **D** | 生文字列として不透明保持 |
| HTML 実体参照（`&amp;` 等） | テキスト | **A** | remark がデコードして保持 |
| バックスラッシュエスケープ | テキスト | **A** | remark が正規に処理 |

### 8.4 変換ロスが発生するケースと UI 対応

変換レベル **C**（近似変換）が発生した場合、ユーザーに通知する。

| ロスのケース | 通知方法 |
|---|---|
| 参照リンクがインラインリンクに展開される | 初回保存時にトースト「参照リンクはインライン形式に変換されます」 |
| setext 見出しが ATX に変換される | 変換時に通知なし（可逆的な正規化として許容） |
| インデントコードがフェンス形式に変換される | 通知なし（表現は変わるが内容は保持） |

**非対応（レベル X）要素は現時点では存在しない**。すべての Markdown 要素について、
少なくとも生文字列としての不透明保持（レベル D）により情報ロスなく往復できる設計とする。
...
意図的に正規化される（保証しない）:
  ⚠️  Markdown 記法の表記バリエーション（`_italic_` → `*italic*` 等）
  ⚠️  参照リンクのリンク定義ブロック（インラインリンクに展開）
  ⚠️  ファイル末尾の余分な空行（1つの改行に統一）
  ⚠️  水平線の記法（`---` に統一）
