...
│                                                         │
│  ┌───────────────────────┐  ┌───────────────────────┐   │
│  │   Markdown WYSIWYG    │  │    HTML WYSIWYG       │   │
│  │  - フォーカス: ソース  │  │  - WYSIWYGモード      │   │
│  │  - 非フォーカス: レンダ│  │  - ソースコードモード  │   │
│  └───────────────────────┘  │  - スプリットモード    │   │
│                              └───────────────────────┘   │
└─────────────────────────────────────────────────────────┘
                        │
...
| クリップボード | **navigator.clipboard** | execCommand | — |
| 数式 | **KaTeX** | MathJax | — |
| 図表 | **Mermaid.js** | — | — |
...
6. **CSS編集の範囲**: HTMLファイル編集時の`<style>`タグ内CSSの扱い
7. **HTMLテンプレート対応**: Jinja2やEJS等のテンプレート構文を含むHTMLの編集
...
12. **プロンプト誤検知対策**: 通常ドキュメントをプロンプトと誤検知するケースの抑制
13. **カスタムテンプレート保存**: ユーザー定義テンプレートのローカル永続化設計
14. **AIプロバイダ連携（将来）**: OpenAI / Anthropic APIをエディタ内から直接呼び出す設計
