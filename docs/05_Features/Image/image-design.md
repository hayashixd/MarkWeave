...
**実装推奨・Phase 3 から有効化**。ユーザーがGUI上でリネーム・移動する場合、古いパスの画像が壊れることは許容できないUX。Typora・Obsidianともに自動更新を実装している。

> **他ドキュメントとの関係（重要）**:
>
> `file-workspace-design.md §5.3` では「MVP ではファイルのリネーム・移動で **他のファイル内の** 相対リンクは自動更新しない」と規定している。この「リンク更新しない」ポリシーは **他ファイルから当該ファイルへの参照** に関するポリシーであり、**移動ファイル自身が内包する画像パス** の更新とは別物である。
>
> | 更新対象 | 対応フェーズ | 仕様箇所 |
> |---------|-----------|---------|
> | 移動した `.md` ファイル **自身の** 画像参照 (`![](./assets/img.png)`) | **Phase 3**（画像挿入機能実装時） | 本セクション §3 |
> | 他の `.md` ファイルから移動ファイルへのリンク (`[text](./note.md)`) | **Phase 7** | `file-workspace-design.md §5.3` |
>
> Phase 3 でファイル移動機能を使った場合、移動されたファイル内の画像パスはこの §3 の `updateImagePathsOnMove` が自動修正する。ただし、**他のファイルが当該ファイルへ張っているリンクは Phase 7 まで更新されない**点はユーザーに対して移動時の確認ダイアログで案内する（`file-workspace-design.md §5.3` 参照）。
