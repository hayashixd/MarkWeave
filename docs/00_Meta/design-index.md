...
| `feature-list.md` | **総合機能一覧・ロードマップ・設計網羅度・実装ログ**（旧 roadmap.md / design-coverage.md / IMPLEMENTATION_LOG.md を統合） | フェーズタスクの追加・完了、設計トピック状態更新、実装ログ追記 |
| `decision-log.md` | 技術選定・アーキテクチャ決定の理由記録（不採用理由・選定根拠） | 新たな重要な技術選定があった際に追記 |
...
| `tauri-ipc-interface.md` | **フロントエンド ↔ バックエンド間の Tauri `invoke()` コマンド型定義書**（引数・戻り値・エラー型の SoT） | **新規 Tauri コマンドを追加する前に必ずここに型定義を追記する**。既存コマンドのシグネチャ変更時も同期更新 |
...
| `menu-inventory.md` | **ネイティブメニューバー全項目一覧（SoT）**。メニュー ID・ショートカット・feature-list 対応・実装先の管理台帳 | メニューへの機能追加時に**先にこのファイルを更新** |
...
2. `feature-list.md` の「設計網羅度一覧」に新トピックの行を追加する（状態: ✅）
3. `feature-list.md` の「設計ドキュメント一覧」テーブルに追加する
