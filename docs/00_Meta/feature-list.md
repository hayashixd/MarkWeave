...
| プラグイン管理パネル | インストール済み一覧・有効/無効切替・アンインストール | 設定 → プラグインタブ | ✅ |
| プラグイン権限確認ダイアログ | インストール前に要求権限と危険権限を確認 | プラグイン追加時に自動表示 | ✅ |
| プラグイン設定動的フォーム | manifest.settings から UI を自動生成して永続化 | 設定 → プラグインタブ | ✅ |
...
| ネイティブメニューバー | Tauri Menu API による OS ネイティブメニュー（ファイル/編集/表示/ヘルプ） | ウィンドウ上部のメニューバーから操作 | ✅ |
...
| BETWEEN...AND 文法堅牢化 | トークナイズ/構文解析改善（P2） | — | ✅ |
| カレンダービュー | Phase 8 スコープ | `VIEW calendar` クエリで表示 | ✅ |
...
| Wikiリンクインデックス | バックエンドでの全スキャン + 差分更新 | 自動（ワークスペース開始時・ファイル保存時） | ✅ |
| バックリンクパネル | リンク元ファイル一覧とコンテキスト引用 | サイドバーの「バックリンク」タブ | ✅ |
| バックリンク自動更新 | ファイル保存時の自動更新 | 自動（ファイル保存時にインデックス更新→パネル再描画） | ✅ |
| リネーム時 Wikiリンク更新 | 確認 UI + 一括更新 + Undo 対応 | ファイルリネーム時に確認ダイアログ→一括更新→Undoトースト | ✅ |
...
| 機能名 | 概要 | 使い方 | 実装 |
|--------|------|--------|------|
| git_status / git_diff コマンド | Rust git2 クレートによる状態取得 | `src-tauri/src/commands/git_commands.rs` | ✅ |
| git_stage / git_unstage / git_commit | ステージング・コミット操作 | Git パネルから操作 | ✅ |
| git_log コマンド | コミット履歴取得 | Git パネル → コミット履歴 | ✅ |
| ファイルツリー Git バッジ | M/U/A/D/C 状態バッジ | ファイルツリーに自動表示 | ✅ |
| エディタガター差分インジケーター | 追加（緑）・変更（橙）・削除（赤） | エディタ左端に自動表示 | ✅ |
| Git パネル | ステージング・コミット UI | メニュー → 表示 → Git パネル、`Ctrl+Shift+7` | ✅ |
| ステータスバーブランチ表示 | ブランチ名・変更件数 | ステータスバー右側に自動表示 | ✅ |
...
| 機能名 | 概要 | 使い方 | 実装 |
|--------|------|--------|------|
| アノテーションモード | 画像ダブルクリックで開始 | 画像をダブルクリック | ✅ |
| 描画ツール | 矩形・楕円・矢印・フリーハンド・テキスト・モザイク・ステップ番号 | ツールバーから選択（ショートカット: R/E/A/F/T/B/N/X） | ✅ |
| アノテーションツールバー | ツール選択・線幅・カラーピッカー | アノテーションモード上部ツールバー | ✅ |
| Undo 機能 | ImageData スナップショット（最大 20 ステップ） | Ctrl+Z / ↩ ボタン | ✅ |
| アノテーション画像保存 | 元画像バックアップ付き | [完了] ボタンで保存（`_original` バックアップ自動作成） | ✅ |
...
| カスタムテンプレート保存 | ユーザー定義テンプレートの永続化 | テンプレートパネルの「+」ボタンで作成・編集・削除 | ✅ |
...
| 国際化（i18n） | 多言語対応（基盤導入済み・英語辞書の本格作成は Phase 5） | [i18n-design.md](../07_Platform_Settings/i18n-design.md) | 🔶 |
...
| Phase 7: 高度な機能 | 49 | 49 | 0 | 100% |
| Phase 7.5: PKM | 30 | 8 | 22 | 27% |
...
### 2026-03-14 セッション #4（SoT 統合更新）
- feature-list（機能一覧）に未記載だった実装済み 3 機能を追記
- 実装進捗サマリーの Phase 7 / 合計件数を再集計して整合
- 旧 roadmap / design-coverage / feature-list / IMPLEMENTATION_LOG を統合した SoT としての整合性を更新
