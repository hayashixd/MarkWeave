...
>
> **注記 — フェーズ番号体系について**:
> 本ドキュメントは Phase 1〜8 の 8 フェーズ体系を採用しており、**正式な実装計画の SoT**。
> `system-design.md §6` は Phase 0〜5 の別体系を使用。主な対応: Phase 1 (本書) ≒ Phase 0 (system-design) / Phase 2 ≒ Phase 1 / Phase 4-5 ≒ Phase 3 / Phase 6-7 ≒ Phase 4 / Phase 8 ≒ Phase 5。
...
- [ ] テーマシステム（CSSベース）
- [ ] PDFエクスポート
- [ ] Pandoc 統合（詳細設計: [pandoc-integration-design.md](./pandoc-integration-design.md)）
...
- [ ] Zen モード用キーボードショートカット（`Ctrl+Shift+Z`）
...
詳細設計: [image-annotation-design.md](./image-annotation-design.md)
...
- [ ] 国際化（i18n）（UI テキストの多言語対応。Phase 5 以降）
...
| [export-design.md](./export-design.md) | HTML/PDF エクスポート設計 |
...
| [wikilinks-backlinks-design.md](./wikilinks-backlinks-design.md) | 双方向リンク（Wikiリンク）・バックリンク設計 |
...
| [image-annotation-design.md](./image-annotation-design.md) | 画像アノテーション（簡易注釈）設計 |
