...
### 基本編集機能（Typora / サクラエディタ基準）

Markdown エディタとして日常利用に必須となる、Typora とサクラエディタの共通的な基本機能。

- [ ] ステータスバー情報表示（文字数・行数・カーソル位置 Ln/Col）
- [ ] エンコーディング表示と切り替え（UTF-8 / Shift_JIS 等）
- [ ] 改行コード表示と切り替え（LF / CRLF）
- [ ] インデント方式設定（Tabs / Spaces・タブ幅）
- [ ] Go To Line（`Ctrl+G`）
- [ ] 現在行ハイライトと対応括弧ハイライト
- [ ] 連番付き「最近使ったファイル」メニュー（`Alt+F`, `1..9` で再オープン）
- [ ] 自動保存（フォーカスアウト / 一定間隔）と変更検知表示
...
### 編集効率化（Typora / サクラエディタの基本操作を補完）

- [ ] 行の複製・行削除（`Ctrl+D`, `Ctrl+Shift+K`）
- [ ] 行移動（`Alt+↑/↓`）
- [ ] 複数カーソル編集（`Alt+Click`, `Ctrl+Alt+↑/↓`）
- [ ] 矩形選択（Alt+ドラッグ）
- [ ] 大文字/小文字変換・全角/半角変換
- [ ] 行ブックマーク（トグル / 次へ / 前へ）
- [ ] 単語補完（文書内候補、`Ctrl+Space`）
- [ ] 空白表示トグル（スペース・タブ・改行記号）
