...
- [x] HTML → Markdown 変換（turndown 統合）
- [x] 変換ロス警告の表示UI（変換できない要素のリスト）
- [x] メニュー: ファイル → 別名で保存 → Markdownとして保存
- [x] メニュー: ファイル → 別名で保存 → HTMLとして保存
- [x] 変換結果を新規タブで開くオプション
...
- [x] テーマシステム（CSSベース）
- [x] PDFエクスポート
- [x] Pandoc 統合（詳細設計: [export-interop-design.md](../06_Export_Interop/export-interop-design.md) §7〜§10）
  - [x] Pandoc インストール確認（起動時 / エクスポート実行時）
  - [x] Pandoc 未インストール時のエラー UX（インストール案内ダイアログ）
  - [x] Word（.docx）エクスポート
  - [x] LaTeX / epub エクスポート
- [x] フォーカスモード
- [x] タイプライターモード
- [x] YAML Front Matter 編集UI（折りたたみ可能パネル、生YAML編集、サマリー表示）
- [x] プラグインAPI
- [x] ワークスペース高度な機能（詳細設計: [file-workspace-design.md](../04_File_Workspace/file-workspace-design.md) §6）
  - [x] ファイルのドラッグ移動
  - [x] リネーム・移動時の Markdown リンク自動更新
  - [x] ワークスペース切り替え（最近使ったワークスペース）

### ウィンドウ管理の拡張

- [x] ウィンドウ状態の記憶・復元（位置・サイズ・最大化状態を次回起動時に復元）
- [x] タブバー空き領域のダブルクリックでウィンドウ最大化トグル + ドラッグ移動
- [x] 最近使ったファイル（Tauri ネイティブメニュー動的更新）
- [x] Windows ジャンプリスト登録（`SHAddToRecentDocs`）
- [x] タブをウィンドウに切り出す機能（WebviewWindow、デスクトップ専用）

### スラッシュコマンド（/）による要素挿入

詳細設計: [slash-commands-design.md](./slash-commands-design.md)

- [x] `SlashCommandsExtension`（TipTap プラグイン実装）
  - [x] トリガー検出ロジック（行頭の `/` のみ）
  - [x] キャラクターマッチングによるフィルタリング（fuse.js の代替実装）
  - [x] コマンド定義（テキスト・見出し・テーブル・コード・AI系 18種）
- [x] スラッシュコマンドメニュー UI コンポーネント
  - [x] カテゴリ別グループ表示
  - [x] キーボード操作（↑↓ で移動、Enter / Tab で実行、Esc で閉じる）
  - [x] ポップアップの位置計算（カーソル直下、ビューポート端での反転）
- [x] AI テンプレートとの統合（`/ブログ` 等のテンプレート直接挿入）
- [x] 設定: `slashCommands.enabled` / `slashCommands.showAiTemplates`（プリファレンスUI統合済み）

### ペイン分割エディタ（Split Editor）

詳細設計: [split-editor-design.md](./split-editor-design.md)

- [x] `SplitEditorLayout` コンポーネント（左右/上下の 2 分割対応）
- [x] スプリッタのドラッグリサイズ（PointerEvents ベース）
- [x] 各ペイン独立タブバー
- [x] `paneStore`（Zustand: ペイン状態・アクティブペイン管理）
- [x] ペイン間フォーカス移動ショートカット（`Ctrl+Alt+←/→`）
- [x] タブをドラッグでペイン間移動
- [x] ペイン状態のセッション保存・復元（`window-tab-session-design.md` と連携）
- [x] 同一ファイル分割時のスクロール同期オプション

### Zen モード（集中モード）強化

詳細設計: [zen-mode-design.md](./zen-mode-design.md)

- [x] Tauri `Window.setFullscreen()` によるフルスクリーン実装
- [x] Zen モード有効時の UI 完全非表示（CSS `.zen-mode` クラス）
- [x] コンテンツ幅・行間・フォントサイズの Zen モード専用設定
- [x] フォーカスモード・タイプライターモードとの統合
- [x] ホバー時のツールバー一時表示（opacity アニメーション）
- [x] 環境音（アンビエントサウンド）再生機能（Web Audio API）
  - [x] ホワイトノイズ・ブラウンノイズ・雨音・カフェの 4 種（外部ファイル不要の純Web Audio合成）
  - [x] 音量コントロール UI（ツールバードロップダウン + 設定タブ）
- [x] タイプライター打鍵音フィードバック（Web Audio API合成、mechanical/soft/typewriterの3スタイル）
- [x] Zen モード用キーボードショートカット（F11 で統合済み）

### 執筆体験強化（一般ライター/ブロガー向け）

ペルソナ: 一般ライター・ブロガー — 執筆リズム・モチベーション維持・文章品質向上

- [x] ポモドーロタイマー（ステータスバー統合: 25分集中→5分休憩のカウントダウン、セッション完了トースト通知）
- [x] ワードスプリント（時間制限付き目標文字数達成モード: 開始→タイマー→達成/未達のサマリー表示）
- [ ] 文章可読性スコア（漢字率・平均文長・読みやすさ指標をステータスバーまたはサイドパネルに表示）
- [ ] スニペット登録・挿入（よく使うフレーズ/文章パターンを登録してスラッシュコマンドから挿入）

### 知識管理強化（知識管理者向け）

ペルソナ: 知識管理者（PKM）— ノート間の関係把握・タグ横断検索・構造可視化

- [ ] タグビュー（ワークスペース全ファイルの YAML FM `tags` を横断収集し、サイドバータブに一覧・タグ別フィルタ）
- [ ] グラフビュー簡易版（開いているタブの Wikiリンク関係を Canvas/SVG で可視化、バックエンド不要）
- [ ] フローティング TOC パネル（右サイドに固定表示する目次、クリックでスクロール、見出し追跡）
- [ ] テンプレートからページ作成（デイリーノート以外: 週次レビュー・プロジェクトメモ等の追加テンプレート）

### AIアシスト機能強化（AIパワーユーザー向け）

ペルソナ: AIパワーユーザー — エディタ上でのシームレスなAI活用

- [ ] テキスト選択 AI アシスト（テキスト選択→ツールチップボタン→改善/翻訳/要約を Claude API で実行）
- [ ] プロンプト構造診断パネル（`analyzePromptStructure()` の RTICCO 結果をサイドバーに常時表示・改善提案）
- [ ] AIコピーの差分プレビュー（最適化前後の Markdown を diff 表示してから確定できるモーダル）

### テクニカルライター/開発者向け強化

ペルソナ: テクニカルライター/開発者 — 技術文書作成効率・コード品質向上

- [ ] Mermaid 図表インライン編集（図表ダブルクリックで Mermaid ソースのポップアップエディタを表示）
- [ ] 数式インライン編集（KaTeX 数式のダブルクリックで LaTeX ソース編集モード）
- [ ] Markdown lint チェック（見出しレベル飛び・リンク切れ・重複見出し等の構造検査、エラー一覧パネル）
- [ ] コードブロックの実行環境情報表示（言語バッジとランナー環境メモをコードブロックヘッダーに表示）
...
- [x] `WikilinkExtension`（TipTap 拡張）
  - [x] `[[ファイル名]]` 記法のパース・NodeView 実装
  - [x] `[[ファイル名|表示テキスト]]` サポート（`[[ファイル名#見出し]]` は将来対応）
  - [x] 解決済み（青色）/ 未解決（赤・波線）の表示分岐（ワークスペースファイルツリーによるフロントエンド判定）
- [x] Wikiリンク オートコンプリート
  - [x] `[[` 入力でファイル候補ポップアップ表示
  - [x] ワークスペースファイルツリーを使った候補一覧（fuse.js 不要の前方一致）
  - [x] LRU ソート（最近開いたファイル順）
- [ ] Wikiリンクインデックス（バックエンド）
  - [ ] Tauri コマンド: ワークスペース全スキャン + 差分更新
  - [ ] `wikilinkStore`（Zustand: インデックス管理・バックリンク提供）
- [x] バックリンクパネル（サイドバータブ、フロントエンドのみ版）
  - [x] リンク元ファイル一覧とコンテキスト引用表示（開いているタブをスキャン）
  - [x] リンク元クリックでジャンプ
  - [ ] ファイル保存時の自動更新（バックエンドインデックスは将来対応）
- [ ] ファイルリネーム時の Wikiリンク自動更新
  - [ ] リネーム後に「XX 件のリンクを更新しますか？」確認 UI
  - [ ] 一括更新 + Undo 対応
- [x] エクスポート時の Wikiリンク → 通常リンク変換（[[target|label]] → [label](target.md)）
...
- [x] `ai-optimizer.ts` 実装（変換パイプライン）
  - [x] `normalizeHeadings()` — 見出し階層の修正
  - [x] `annotateCodeBlocks()` — コードブロックへの言語タグ付与（言語推定）
  - [x] `normalizeListMarkers()` — リスト記号の統一
  - [x] `trimExcessiveWhitespace()` — 過剰空白行の削除
  - [x] `annotateLinks()` — リンクへのURL注記
  - [x] `normalizeCodeFences()` — コードフェンスの統一
  - [x] `analyzePromptStructure()` — RTICCO構造の検出・診断
- [x] `optimizeAndCopy()` — クリップボードへのコピー実装
- [x] `buildReport()` — 変更点レポート生成
- [x] ツールバーへの **[AIコピー]** ボタン配置
  - [x] シングルクリック: 最適化してコピー
  - [x] ドロップダウン: プレビュー表示 / オプション設定
- [x] 最適化レポート ポップオーバーUI
- [x] 各変換のオン/オフ設定（オプションメニュー）

### AIテンプレートシステム

- [x] テンプレートレジストリ実装（`template-registry.ts`）
  - [x] `registerTemplate()` — テンプレート登録
  - [x] `listTemplates()` / `searchTemplates()` — 一覧・検索
  - [x] `fillTemplate()` — プレースホルダー置換
- [x] 組み込みテンプレート（6種）
  - [x] ブログ構成案
  - [x] コード解説（言語選択対応）
  - [x] コードレビュー依頼
  - [x] 要約用プロンプト
  - [x] 思考の連鎖（Chain of Thought）
  - [x] 会議メモ整形
- [x] テンプレートパネルUI
  - [x] カテゴリフィルタ（ブログ / コード / 要約 / 推論 / 議事録 / 全て）
  - [x] キーワード検索
  - [x] テンプレートプレビュー
  - [x] プレースホルダー入力ダイアログ
  - [x] カーソル挿入 / ドキュメント全体置換の選択
- [ ] カスタムテンプレートの保存・管理（将来）
