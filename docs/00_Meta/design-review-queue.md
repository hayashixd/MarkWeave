...
- レビューで指摘した未実装 / 不一致に対する実装対応が完了したら、対象行の **実装対応** を `対応済み（YYYY-MM-DD）` に更新する。
- あわせてレビューファイルを `docs/00_Meta/bk/` へ移動し、**レビューファイル** 列のパスを `bk/` 配下に更新する。
- 「レビュー完了（done）」だけでは `対応済み` にしない。必ず実装対応の完了を確認してから更新する。
