...
/// モデル名の書式: 日付サフィックスなし = 最新バージョンを指す（APIがエイリアスを解決）
///                日付サフィックスあり = 特定バージョンに固定（互換性が重要な場合）
const ALLOWED_MODELS: &[(&str, &str)] = &[
    ("anthropic", "claude-sonnet-4-5"),           // 最新エイリアス
    ("anthropic", "claude-haiku-4-5-20251001"),   // バージョン固定
    ("openai",    "gpt-4o"),                       // 最新エイリアス
    ("openai",    "gpt-4o-mini"),                  // 最新エイリアス
];
...
> **本セクションは外部プラグイン（サードパーティ製コミュニティプラグイン）のみを対象とする。**
> ビルトインプラグイン（Mermaid・KaTeX・画像等、アプリ同梱）は Phase 3 から実装するが、
> 外部プラグインシステムは **Phase 1-6 では実装しない**。
> Phase 7 以降でコミュニティプラグインを検討する場合のポリシー草案。
>
> 区別:
> - **ビルトインプラグイン**（Phase 3〜）: アプリ同梱・開発者がレビュー済み・`plugin-api-design.md §5` 参照
> - **外部プラグイン**（Phase 7〜）: サードパーティ製・本セクションのレビューポリシーが適用される
