...
> **設計判断: レイヤー分離型 vs 機能駆動型**
>
> フロントエンドの構成は「**レイヤー分離型**」（`core/`, `renderer/`, `components/` で技術層ごとに分割）を採用する。
> 機能駆動型パッケージング（`features/editor/`, `features/workspace/` 等でドメイン単位にまとめる方式）も検討したが、
> 以下の理由によりレイヤー分離型を選択した:
...
└── src/
    ...
    │   ├── ai_commands.rs        # セキュアな外部 AI API 通信のプロキシ（APIキーを Rust で隠蔽）
