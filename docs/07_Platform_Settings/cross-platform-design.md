...
| Phase 5 | **macOS** (Ventura 以降) | デスクトップ拡張 |
| Phase 5 | **Linux** (Ubuntu 22.04+) | デスクトップ拡張 |
| Phase 5 | **Android** (API 30 以降) | モバイル第一弾 |
| Phase 5 | **iOS** (iOS 16 以降) | モバイル第二弾 |
...
- Windows Phase 1 では WebKit 対応は後回しにしてよい。Phase 5 で一括対処する
...
| iOS (Sandbox) | Documents フォルダ経由のファイルは通常 UTF-8。Shift-JIS ファイルを扱うユーザーはほぼデスクトップ利用が前提 | PC 向けと同じ chardet ロジックを適用。誤判定時の手動指定 UI は Phase 5 以降で追加 |
...
| E2Eテスト（モバイル） | Appium or BrowserStack | Android/iOS（手動、Phase 5）|
...
|----------|---------|-------|---------|-----|
| ファイル開く・保存 | ✅ E2E | Phase 5 | Phase 5 | Phase 5 |
| 日本語 IME 入力 | ✅ E2E | Phase 5 | Phase 5 | Phase 5 |
| キーボードショートカット | ✅ E2E | Phase 5 | — | — |
| タッチ操作（基本） | — | — | Phase 5 | Phase 5 |
| ファイル関連付け | ✅ 手動 | Phase 5 | Phase 5 | Phase 5 |
| セッション復元 | ✅ E2E | Phase 5 | Phase 5 | Phase 5 |
| 仮想キーボード共存 | — | — | Phase 5 | Phase 5 |
...
*このドキュメントは Phase 5（マルチプラットフォーム展開）に向けて随時更新する。*
