...
## 🔍 回答・コード品質プロトコル（出力前に必ず実行）

コードや説明・コメントを生成したら、**ユーザーに提示する前に**以下のサイクルを必ず実行する。
「書いた」で終わりにせず、「直した上で出す」を習慣にすること。

### フェーズ A: 批判的レビュー（意図的に粗探しをする）

コードや回答を一度脇に置き、**疑いの目で**以下を確認する。
問題が見つかれば即修正してから次へ進む。
...
```
1回目: 「要件・設計書を完全に満たしているか？」— 仕様の抜け・漏れを探す
2回目: 「もっとシンプルに書けないか？」— 不要な複雑さを削る
3回目: 「まだ見落としているバグ・エッジケースはないか？」— 別の角度から目を通す
```
...
> 採点結果をユーザーに見せる必要はない。あくまで自分への品質チェックとして使う。
...
- **上記以外の新規ライブラリ全般** → 必要性がある場合はユーザーに相談してから追加する
...
- Tauri API を使う関数は `src/lib/tauri-commands.test.ts` のように `vi.mock` でモックする
...
- `e2e/app.spec.ts` に基本操作の E2E テストを追加する
...
```
feat(phase-N): <実装したタスク名>
```

例: `feat(phase-2): テーブルのレンダリング`
